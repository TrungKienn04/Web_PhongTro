...
const ManagePosts = () => {
  ...
  const categoryMap = useMemo(
    () => Object.fromEntries((categories || []).map((item) => [item.code, item.value])),
    [categories],
  );
  ...
  const openEditor = useCallback((postId) => {
    setSearchParams(updateEditorSearchParams(searchParams, postId));
  }, [searchParams, setSearchParams]);

  const closeEditor = useCallback((replace = true) => {
  ...
  useEffect(() => {
    ...
    setErrors({});
  }, [activePost, activePostId, closeEditor, loading]);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await apiUpdatePost(activePostId, toPostPayload(formData));
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await apiDeletePost(deleteTarget.id);
      ...
      if (response?.data?.err === 0) {
        ...
        await Swal.fire({
          ...
          text: "Bài đăng đã được gỡ khỏi danh sách quản lý và dữ liệu hiển thị liên quan.",
        });
        return;
      }
    ...
    return (
      <article
        ...
        className={`${postCardClass} ${
          isActive ? "border-slate-800 shadow-[0_14px_32px_rgba(15,23,42,0.08)]" : ""
        }`}
      >
        <div
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-col">
              ...
              <div className="min-w-0">
                ...
                <h3
                  className={`mt-2 font-bold text-slate-950 ${
                    expanded ? "text-2xl leading-tight" : "line-clamp-2 text-lg leading-8"
                  }`}
                >
                  {post.title}
                </h3>
            ...
            <div className="mt-3 space-y-2 text-sm text-slate-500">
              <p className={expanded ? "text-base" : "line-clamp-2"}>{post.address}</p>
            ...
            <div
              className={`mt-auto grid gap-2 pt-5 ${
                expanded ? "sm:max-w-[420px] sm:grid-cols-3" : "grid-cols-3"
              }`}
            >
              <button
                type="button"
  ...
  return (
    <div className="space-y-5">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-500">
                    Số lượng tin đăng:{" "}
                    <span className="font-semibold text-slate-900">{posts.length} tin</span>
                  </p>
                </div>
      ...
      <ConfirmDialog
        ...
        title="Xác nhận xóa"
        description={`Bài đăng "${deleteTarget?.title || ""}" sẽ bị xóa khỏi danh sách quản lý và dữ liệu hiển thị liên quan. Thao tác này không thể hoàn tác.`}
