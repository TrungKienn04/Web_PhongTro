...
const { getDisplayDescription, getPrimaryImage } = require("../../ultils/Common/postHelpers");
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="space-y-5 rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm lg:p-8">
        <div className="space-y-3">
          ...
          <h1 className="text-3xl font-semibold leading-tight text-slate-900 break-words">
            {post.title}
          </h1>
          ...
          <div className="space-y-3">
            ...
            {Array.isArray(post.description) ? (
              post.description.map((item, index) => (
                <p key={index} className="text-sm leading-7 text-slate-600 break-words">
                  {item}
                </p>
              ))
            ) : (
      ...
      <aside className="space-y-5">
        ...
        <div className="rounded-[32px] border border-slate-200 bg-gradient-to-br from-amber-50 to-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm leading-7 text-slate-500">
            Tin đang hiển thị đầy đủ thông tin, phù hợp để gọi ngay hoặc nhắn Zalo trước khi đi xem phòng.
          </p>
        </div>
      </aside>
    </div>
